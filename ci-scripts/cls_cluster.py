	...
	def BuildClusterImage(self, HTML):
		...
		if self.ranAllowMerge: # merging MR branch into develop -> temporary image
			...
			if self.ranTargetBranch == 'develop':
				...
				mySSH.command(f'git diff {self.ranCommitID}..origin/develop -- cmake_targets/build_oai cmake_targets/tools/build_helper | grep --colour=never -i INDEX', '\$', 5)
				#mySSH.command(f'git diff {self.ranCommitID}..origin/develop -- cmake_targets/build_oai cmake_targets/tools/build_helper docker/Dockerfile.base.rhel8.2 | grep --colour=never -i INDEX', '\$', 5)
