...
#-----------------------------------------------------------
	...
	def BuildImage(self, HTML):
		...
		if self.host == 'Red Hat':
			...
			mySSH.command('sudo cp /etc/rhsm.conf ./rhsm-conf/', '\$', 5)
		...
		if (self.ranAllowMerge):
			...
			if self.ranTargetBranch == 'develop':
				mySSH.command(f'git diff {self.ranCommitID}..origin/develop -- cmake_targets/build_oai cmake_targets/tools/build_helper docker/Dockerfile.base' + self.dockerfileprefix + ' | grep --colour=never -i INDEX', '\$', 5)
